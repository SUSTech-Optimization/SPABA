...
class Solver(BaseSolver): 
    ...
    def run(self, callback):
        ...
        if self.framework == "jax":
            ...
            v1 = -jnp.ones_like(inner_var)
            v2 = jnp.ones_like(inner_var)
            inner_var1 = -jnp.ones_like(inner_var)
            inner_var2 = jnp.ones_like(inner_var)
            outer_var1 = -jnp.ones_like(outer_var)
            outer_var2 = jnp.ones_like(outer_var)
            # Init lr scheduler
...
def soba_jax(f_inner, f_outer, f_inner1, f_outer1, inner_var, outer_var, v, v1, v2, inner_var1, inner_var2, outer_var1, outer_var2,
    ...
    def soba_one_iter(carry, _):
        ...
        carry['v'] -= inner_step_size * (hvp1 - grad_in_outer1)
        carry['outer_var'] -= outer_step_size * (-cross_v1 + grad_out_outer1)
