...
class Solver(BaseSolver):
    ...
    def run(self, callback):
        ...
        if self.framework == 'jax':
            ...
            memory = self.init_memory(
                self.f_inner, self.f_outer,
                inner_var, outer_var, v, q, inner_sampler, outer_sampler,
        ...
        while callback():
            if self.framework == 'jax':
                inner_var, outer_var, v, q,memory, carry = self.saba(
                    self.f_inner, self.f_outer,
                    inner_var, outer_var, v, q,memory,
                    max_iter=eval_freq, **carry
                )
            else:
                inner_var, outer_var, v ,q = self.saba(
                    self.f_inner, self.f_outer,
                    inner_var, outer_var, v, q, memory,
...
def _init_memory_fb(memory, inner_oracle, outer_oracle,
    ...
    for _ in prange(n_inner):
        ...
        _, grad_inner_var, hvp, cross_v = inner_oracle.oracles(
            inner_var, outer_var, v, q, slice_inner, inverse='id'
        )
...
def _saba(variance_reduction, inner_oracle, outer_oracle, inner_var, outer_var,
          v,q, memory, inner_sampler=None, outer_sampler=None, lr_scheduler=None,
    ...
    for i in range(max_iter):
        ...
        v -= outer_step_size * (-hvp + grad_in_outer)
        q = (1-outer_step_size)*q + outer_step_size * (-cross_v + grad_out_outer)
        outer_var -= outer_step_size * q
...
def saba_jax(f_inner, f_outer, inner_var, outer_var, v, q,memory,
    ...
    init = dict(
        inner_var=inner_var, outer_var=outer_var, v=v, q=q,
    ...
    return (
        carry['inner_var'], carry['outer_var'], carry['v'], carry['q'],
        memory,
        {k: v for k, v in carry.items()
         if k not in ['inner_var', 'outer_var', 'v', 'q']}
    )
