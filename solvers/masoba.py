...
class Solver(BaseSolver):
    ...
    def run(self, callback):
        ...
        while callback():
            if self.framework == 'jax':
                inner_var, outer_var, v, q, carry = self.soba(
                    self.f_inner, self.f_outer,
                    inner_var, outer_var, v, q, max_iter=eval_freq, **carry
                )
            else:
...
def soba(inner_oracle, outer_oracle, inner_var, outer_var, v, q,
...
def soba_jax(f_inner, f_outer, inner_var, outer_var, v, q,
    ...
    def soba_one_iter(carry, _):
        ...
        carry['q'] = 0.1*(cross_v + grad_out_outer) + (1-0.1)*carry['q']
        carry['inner_var'] -= inner_step_size * grad_inner_var
        carry['v'] -= inner_step_size * (hvp + grad_in_outer)
        carry['outer_var'] -= outer_step_size * carry['q']
    ...
    init = dict(
        inner_var=inner_var, outer_var=outer_var, v=v, q=q, state_lr=state_lr,
